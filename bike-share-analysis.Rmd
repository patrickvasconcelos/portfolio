...
Our historical data is in this [link](https://divvy-tripdata.s3.amazonaws.com/index.html), under this [license](https://ride.divvybikes.com/data-license-agreement). They are in 12 CSVs containing details of the bike trips for each month of 2021, and it's organized in the following way:
...
> Disclaimer: The data is actually from [Divvy Bikes](https://ride.divvybikes.com/), an actual bike-sharing service in Chicago.
...
To make our analysis easier, we have to ordinate the month and the weekdays:
...
Our final database look like this:

```{r aggregated data, echo=FALSE}
knitr::kable(head(bike_data), "pipe")
```
...
Also, we look for patterns in the weekdays:
...
We can search for trends of use in different types of bikes, so let's generate this base:
...
After performed our analysis, we will develop some data visualizations to communicate our findings to the executive team. To do that, we will need to be concise and precise on our decisions to what use on the presentation.
...
rides_per_user_weekday %>%
  ...
  geom_label(aes(x = 5, y = 10, label = "Cyclistic members"), 
             ...
             size = 5)
```
We can see that Cyclistic members make the shortest rides.
...
- Create new fidelity plan, so customer who rides longer have some discount when he is a Cyclistic member;
- Create a month plan, to attend the high pic of rides in the middle of the year.

As further analysis we can use data from the customers, such as job occupation and how they use our bikes, to understand why the uses on weekend is so high.
